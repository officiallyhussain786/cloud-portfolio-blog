...
pythonasync def process_document(doc):
...
gofunc readDocs(ctx context.Context, docs []string) <-chan Document {
...
gofunc chunkWorker(ctx context.Context, in <-chan Document, out chan<- Document, wg *sync.WaitGroup, id int) {
    defer wg.Done()
    for doc := range in {
        select {
        case <-ctx.Done():
...
gofunc embedDocs(ctx context.Context, in <-chan Document) <-chan Document {
    out := make(chan Document)
