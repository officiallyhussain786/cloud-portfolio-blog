...
description: "Learn how Go's Pipeline, Fan Out, and Fan In concurrency patterns 
solve the async problems that break Python at scale — with real 
code from a production document ingestion pipeline."
pubDate: 2026-03-11
category: "Systems"
---
...
Fan Out     → one input channel, many workers
              work distributes itself — whoever is free picks next job
              Go scheduler handles distribution automatically

Fan In      → many workers, one output channel
              results collect themselves into one stream
              WaitGroup + close() signal when collection is done
```
